...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Run the explicitly approximate Briggs (2017) pragmatic analogue."
    )
    p.add_argument("--config", default="config/briggs_2017_pragmatic_analogue.json")
    p.add_argument(
        "--analysis-frame",
        required=True,
        help="Local Parquet or CSV with independently reconstructed regional quantities",
    )
